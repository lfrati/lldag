...
class Value:
    ...
    def relu(self):
        cmp = builder.fcmp_ordered("<=", zerof, self.data, flags=("fast",))
        data = builder.select(cmp, self.data, zerof)
        return Value(data, _children=(self,), _op="ReLU")
...
if __name__ == "__main__":
    ...
    for i, retval in enumerate(retvals):
        builder.store(retval.data, builder.gep(fargs[-1], [index(i)], inbounds=True))
    builder.ret(zeroi)
